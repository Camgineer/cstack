...
Plugin defaults live in `policy/models.md`. They return on every plugin update.
...
6. Write `~/.agents/cstack-models.md` with only the roles that differ from `policy/models.md`. Keep overlay roles the user did not change.
7. Tell the user the overlay path and that plugin updates will not touch it.

## Overlay shape

```
# C-Stack model overlay. Named roles replace policy/models.md. Delete a line to restore the plugin default.
