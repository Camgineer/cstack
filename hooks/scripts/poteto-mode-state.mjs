...
const ACTIVATION = /^\s*\$(?:cstack:)?poteto-mode(?:\s|$)/u;
const LINKED_ACTIVATION = /^\s*\[\$(?:cstack:)?poteto-mode\]\((\/[^()\\\x00-\x1f]+\/skills\/poteto-mode\/SKILL\.md)\)(?:\s|$)/u;
...
export async function collectExpired(
  ...
  for (const directory of [path.join(root, "sessions"), path.join(root, "receipts")]) {
    ...
    await runBounded(candidates, concurrency, async (entry) => {
      ...
      if (value?.schema === STATE_SCHEMA && value?.active === false) return;
...
export async function readActiveState({ pluginData, sessionId, cwd, now = Date.now(), ttlMs = DEFAULT_TTL_MS }) {
  ...
  if (state?.schema === STATE_SCHEMA && state?.active === false) return null;
...
export async function handleHook(input, options = {}) {
  ...
  if (event === "SessionStart") {
    ...
    return {
      hookSpecificOutput: {
        ...
        additionalContext: "C-Stack remains active for this resumed or compacted session. Apply $cstack:poteto-mode, $cstack:simple-as-writing, policy/coding-standards.md, and policy/cstack-models.md. Named roles in ~/.agents/cstack-models.md win when that file exists. Do not infer authority beyond the user request.",
      },
    };
  }
  if (event !== "UserPromptSubmit") return null;

  let action = classifyPrompt(input.prompt);
  if (options.defaultActive && action === "inactive") {
    const saved = await readJson(targets.state);
    const optedOut = saved?.schema === STATE_SCHEMA && saved?.active === false;
    if (!optedOut) action = "activate";
  }
  if (action === "disable") {
    if (options.defaultActive) {
      await atomicWrite(targets.state, { ...stateValue(fingerprint, now), active: false });
      await removeFile(targets.receipt);
    } else {
  ...
  if (action === "activate") {
    ...
    return {
      hookSpecificOutput: {
        ...
        additionalContext: "C-Stack defaults are active. Apply $cstack:poteto-mode, $cstack:simple-as-writing, policy/coding-standards.md, and policy/cstack-models.md. Named roles in ~/.agents/cstack-models.md win when that file exists. Default activation does not expand user authority.",
      },
    };
  }
  ...
  return {
    hookSpecificOutput: {
      hookEventName: "UserPromptSubmit",
      additionalContext: "C-Stack is active for this session. Apply $cstack:poteto-mode, $cstack:simple-as-writing, policy/coding-standards.md, and policy/cstack-models.md. Named roles in ~/.agents/cstack-models.md win when that file exists. Do not infer authority beyond the user request.",
    },
  };
}
